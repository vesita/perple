...
use crate::{cloud::CldBud, config::{fixif, Config}};
...
pub fn pick_ground(cloud: &mut [[f32; 3]]) -> (usize, Vec<CldBud>) {
    ...
    // 获取配置参数（旧版 pick_ground 兼容）
    let ground_filter_threshold = 0.3;
    let cross_product_patience = 3.0;
    let sample_test_count = 23;
...
/// 基于直方图种子 + RANSAC 平面拟合生长的地面/天花板检测
///
/// 策略：
/// 1. 按 Z 排序点云，建直方图找峰值 → 种子区域 (peak_z ± ground_expand)
/// 2. 在种子区域上运行轻量 RANSAC，找最佳平面
/// 3. 将最佳平面生长到全点云
...
pub fn single_pick_ground(cloud: &mut [[f32; 3]]) -> (usize, Vec<CldBud>) {
    ...
    // 倒置 LiDAR：Z 轴朝下，地面在 HIGH Z 端
    // 取反 Z 使地面回到 LOW Z 端，统一处理逻辑
    if ixi.upside_down {
    ...
    // 2. 找地面峰值
    //    倒置模式下 ground 在 LOW Z 端（取反后），从底部向上扫描找第一个显著 bin
    //    正常模式下取全局最高峰
    let peak = if ixi.upside_down {
    ...
    let expand = ixi.ground_expand;
    let z_low = peak_z - expand;
    let z_high = peak_z + expand;

    // 种子区域 = Z 直方图峰值附近 ± expand
    ...
    if ixi.has_ceiling && n_ground < n {
        let remaining = &cloud[n_ground..];
        let rn = remaining.len();
        let rz_min = remaining[0][2];
        let rz_max = remaining[rn - 1][2];
        let rz_range = rz_max - rz_min;

        if rz_range > 1e-6 && rn > 10 {
            let ceil_bin_w = rz_range / num_bins as f32;
            let mut ceil_bins = vec![0usize; num_bins];
            for p in remaining.iter() {
                let mut b = ((p[2] - rz_min) / ceil_bin_w) as usize;
                b = b.min(num_bins - 1);
                ceil_bins[b] += 1;
            }

            let ceil_peak = ceil_bins.iter().enumerate()
                .max_by_key(|(_, c)| *c)
                .map(|(i, _)| i)
                .unwrap_or(0);
            let ceil_peak_z = rz_min + (ceil_peak as f32 + 0.5) * ceil_bin_w;
            let ceil_z_low = ceil_peak_z - expand;
            let ceil_z_high = ceil_peak_z + expand;

            let ceil_points: Vec<[f32; 3]> = remaining.iter()
                .filter(|p| p[2] >= ceil_z_low && p[2] <= ceil_z_high)
                .copied()
                .collect();

            if !ceil_points.is_empty() {
                let mut ceil_box = Box3D::empty_box();
                ceil_box.cloud2box(&ceil_points);
                all_grounds.push(CldBud::new(ceil_box, 1, "ceiling".to_string(), 0.8));
            }
        }
    }
...
fn histoseed_plane(
    ...
    eprintln!(
        ...
        n_ground as f64 / n as f64 * 100.0,
    );
