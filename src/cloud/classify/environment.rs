...
pub fn pick_ground(cloud: &mut [[f32; 3]]) -> (usize, Vec<CldBud>) {
    ...
    for i in trusted_end..cloud.len() {
        ...
        if can_add_by_normal_check_optimized(trusted_points, cloud[i], cross_product_patience, sample_test_count) {
            added[i] = true;
    ...
    let ground_points: Vec<[f32; 3]> = cloud.iter()
        .enumerate()
        .filter(|(i, _)| added[*i])
        .map(|(_, point)| *point)
        .collect();
...
pub fn pick_wall(cloud: &mut [[f32; 3]]) -> (usize, Vec<CldBud>) {
    ...
    loop {
        ...
        // 收集未被使用的点的索引
        let unused_indices: Vec<usize> = indices.iter()
            .filter(|&&i| !used[i])
            .copied()
            .collect();
        ...
        for _ in 0..ransac_iterations {
            ...
            let sampled_indices: Vec<usize> = sampled_indices_raw.iter()
            ...
            for &idx in &unused_indices {
                ...
                let dist = (normalized_normal[0] * point[0] + 
                           ...
                           normalized_normal[2] * point[2] - distance).abs();
        ...
        if best_support_count < min_wall_points && min_wall_points > 15 {
            ...
            // 再次尝试寻找满足放宽条件的墙面
            for _ in 0..(ransac_iterations*2) { // 增加迭代次数
                // 使用select_some函数随机选择3个不重复的点
                let sampled_indices_raw = select_some(0, unused_indices.len(), 3);
                if sampled_indices_raw.len() < 3 {
                    continue;
                ...
                if dot_with_z > vertical_tolerance * 2.0 { // 进一步放宽垂直度要求
                    continue;
                }
        ...
        // 根据最佳平面提取墙面点
        let wall_points: Vec<[f32; 3]> = best_inliers.iter()
            .map(|&i| cloud[i])
            .collect();
        ...
        let wall_cld_bud = CldBud::new(
            ...
            if best_support_count >= min_wall_points { 0.9 } 
            else if best_support_count >= min_wall_points/2 { 0.7 } 
            else { 0.5 }, // 根据点数确定置信度
        );
            ...
            if sorted_by_x.len() > 20 {
                ...
                if x_range > 0.1 { // 确保有足够的范围
                    ...
                    for (bin_idx, bin) in bins.iter().enumerate() {
                        if bin.len() >= 20 { // 如果某个bin中有足够多的点
                            let wall_points: Vec<[f32; 3]> = bin.iter().map(|&i| cloud[i]).collect();
...
fn aboat_normal(plane: [[f32; 3]; 3], reference_normal: [f32; 3]) -> f32 {
    // 计算两个边向量
    let edge1 = [
        plane[1][0] - plane[0][0],
        plane[1][1] - plane[0][1],
...
/// 检查点集是否满足平面分布要求
fn satisfy_plane(points: &[[f32; 3]], tolerance: f32, sample_test_count: usize) -> bool {
    if points.len() < 3 {
        return false;
    }
...
fn can_add_by_normal_check(trusted_points: &[[f32; 3]], new_point: [f32; 3], tolerance: f32, sample_test_count: usize) -> bool {
    ...
    for _ in 0..sample_test_count {
        ...
        if sample_indices.len() >= 3 {
            let samples: Vec<[f32; 3]> = sample_indices.iter().map(|&i| trusted_points[i]).collect();
            ...
            if sample_indices2.len() >= 2 {
                let samples2: Vec<[f32; 3]> = sample_indices2.iter().map(|&i| trusted_points[i]).collect();
...
/// 检查新点是否可以通过法向量检验加入到现有点集中
fn can_add_by_normal_check_optimized(trusted_points: &[[f32; 3]], new_point: [f32; 3], tolerance: f32, sample_test_count: usize) -> bool {
    if trusted_points.len() < 3 {
        return false;
    }
...
/// 检查新点是否可以加入到现有平面中
fn can_add_to_plane(existing_points: &[[f32; 3]], new_point: [f32; 3], tolerance: f32, sample_test_count: usize) -> bool {
    // 创建包含新点的临时点集
