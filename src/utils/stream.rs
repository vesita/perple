...
impl<T: Default + Send + Clone> Stream<T> {
    ...
    pub fn get_write_mut(&mut self) -> Result<&mut Option<T>, StreamError> {
        ...
        // 安全地获取可变引用
        Ok(unsafe { self.pool[current_write].assume_init_mut() })
    }
    ...
    pub fn get_read_ref(&self) -> Option<&Option<T>> {
        ...
        // 安全地获取不可变引用
        Some(unsafe { self.pool[current_read].assume_init_ref() })
    }
    ...
    pub fn commit_read(&mut self) -> Result<(), StreamError> {
        ...
        // 更新读索引
        self.read_index
            .store((current_read + 1) % self.capacity, Ordering::Release);
        Ok(())
    }
    ...
    pub fn read_indexed(&mut self) -> Option<(T, usize)> {
        ...
        // 安全地读取数据
        let data = unsafe { self.pool[current_read].assume_init_read() };
