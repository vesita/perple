...
fn apply_nms(detections: &mut Vec<ClrBud>, nms_threshold: f32) -> Vec<ClrBud> {
    ...
    for i in 0..detections.len() {
        ...
        // 缓存当前检测框的面积以避免重复计算
        let area_i = (detections[i].the_box.x2 - detections[i].the_box.x1)
            * (detections[i].the_box.y2 - detections[i].the_box.y1);
        ...
        for j in (i + 1)..detections.len() {
            ...
            let area_j = (detections[j].the_box.x2 - detections[j].the_box.x1)
                * (detections[j].the_box.y2 - detections[j].the_box.y1);
