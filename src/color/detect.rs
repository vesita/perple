use crate::{
    color::{
        ClrBud,
        image::{ScaleMessage, image_to_tensor, resize_image},
        load_model,
        utils::{nms_tensor, to_input},
    },
    config::fixif,
};
...
impl YoloDetector {
    ...
    pub fn detect(
        &mut self,
        image: &DynamicImage,
    ) -> Result<Vec<ClrBud>, Box<dyn std::error::Error>> {
        // 调整图像大小
    ...
    pub fn detect_batch(
        &mut self,
        images: &[DynamicImage],
    ) -> Result<Vec<Vec<ClrBud>>, Box<dyn std::error::Error>> {
